...
def _sanitize_content_for_file(content: str, max_length: int = 2000) -> str:
    """Sanitize content before writing to markdown files: single line, no injection of structure."""
    if not content:
        return ""
    # Replace newlines with space so we don't break list structure or inject headings
    one_line = " ".join(content.split())
    return one_line[:max_length] if len(one_line) > max_length else one_line
...
"""
    ...
    # Format the entry (sanitize content for markdown so newlines don't break structure)
    content_safe = _sanitize_content_for_file(content)
    ...
    if timestamp:
        entry_line = f"- {time_str} {type_prefix} {content_safe}{category_tag}\n"
    else:
        entry_line = f"- {type_prefix} {content_safe}{category_tag}\n"
...
def append_to_memory_file(
    ...
    for i, line in enumerate(lines):
        ...
        if found_section and not inserted:
            ...
            if line.startswith('## ') or (line.strip() == '' and i + 1 < len(lines) and lines[i + 1].startswith('## ')):
                ...
                new_lines.insert(-1, f"- {content_safe}")
                inserted = True
            elif line.strip() == '---':
    ...
    if found_section and not inserted:
        new_lines.append(f"- {content_safe}")
        inserted = True
